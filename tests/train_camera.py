"""
Training script for the camera-based line following agent.
"""
...
def main():
    """Train the line following agent."""
    print("Initializing camera-based line following training...")
    ...
    # Create environment (start with feature-based observations)
    print("\nCreating environment...")
    print("  Mode: Feature-based observations (faster training)")
    print("  Track width: 60cm")
    env = CameraLineFollowEnv(render_mode=None, use_raw_pixels=False)
    env = Monitor(env, LOGGING_CONFIG['log_dir'])
    
    # Create evaluation environment
    eval_env = CameraLineFollowEnv(render_mode=None, use_raw_pixels=False)
    ...
    checkpoint_callback = CheckpointCallback(
        ...
        name_prefix='camera_line_follow_model'
    )
    ...
    if HUGGINGFACE_CONFIG['enabled'] and HUGGINGFACE_CONFIG['repo_id']:
        ...
        print("   To enable, set HUGGINGFACE_CONFIG['enabled'] = True in config/config.py")
    ...
    # Save final model
    final_model_path = Path(LOGGING_CONFIG['checkpoint_dir']) / 'camera_line_follow_final'
    ...
    if HUGGINGFACE_CONFIG['enabled'] and HUGGINGFACE_CONFIG['repo_id'] and HUGGINGFACE_CONFIG['push_on_complete']:
        ...
        push_final_model(
            model=model,
            repo_id=HUGGINGFACE_CONFIG['repo_id'],
            model_name="camera_line_follow_final",
            private=HUGGINGFACE_CONFIG['private'],
            commit_message=f"Final line following model after {TRAINING_CONFIG['total_timesteps']:,} timesteps"
        )
        
        # Create model card
        model_info = {
            'name': 'CarV1 Camera-Based Line Following Agent',
            'algorithm': 'PPO',
        }
        ...
        training_info = {
            ...
            'environment': 'Camera-based line following with white rope boundaries',
            'observation_space': 'Features: [left_offset, right_offset, heading, speed]',
        }
    ...
    print("  1. Visualize trained agent: python tests/test_trained_model.py")
    print("  2. Test on real hardware with Raspberry Pi deployment")
    print("="*60)
